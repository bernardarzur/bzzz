...
print (' configuration 106 : RX ET TX, ON VA CONFIGURER LE RX; il y a 0 capteur sur le RX; il y a y capteurs sur le TX'), ajoute timestamp et causes du réveil deepsleep
...
print (' configuration 300 : RX: ON VA COPIER le fichier_data de la flash sur le pc')
"""
...
#import machine#on voulait fixer la freq horloge du Lopy à 160 MHz, mais.....
#############################################################################################################################
#############################################################################################################################
#############################################################################################################################
configuration=206
debug=1
wake=0
t_input=(2018, 3, 2, 15, 59, 0, 0, 0)# on peut remettre le RX à l'heure, si le temps archivé sur la flash est inférieur au temps input, sinon on garde le temps flash qui efface partiellement les reboots du RX

print ('configuration:', configuration,  'debug:',  debug, 'mise en sommeil: ', wake,'temps_input: ',   t_input)
...
def flashWriteData(trame) : #on ajoute trame dans fichier data
...
def flashWriteTime(trame) : #on écrit trame dans fichier data
     ofi=open('fichier_time', 'w')
     ofi.write(trame)
     ofi.close()
     return

def flashReadTime() : #on lit data dans fichier data
     ofi=open('fichier_time', 'r')
     t=ofi.read()
     ofi.close()
     return t
...
t_flash_int=[0]*8                                                          #temps (y,m,d,h,mn,s,us,none)  
...
if configuration== 106: # ON VA CONFIGURER LE RX il y a 0 capteurs sur le RX; il y a i capteurs sur le TX;   trame=  1 trame TX+ 1T local+ timestamp
    rtc = RTC()
    t_ch=flashReadTime()
    t_flash=t_ch.split(delimiteur) #on vire le delimiteur et on met les data dans une liste
    for i in range(0, 7):
        t_flash_int[i]=int(t_flash[i])
    rtc.init(t_input)#initialisation de RTC avec t_init
    for i in range(0, 5):#on regarde si t_flash est posterieur à t_init
        if t_input[i]<int(t_flash_int[i]):
            rtc.init(t_flash_int)#initialisation de RTC avec t_flash
            break
    ts=rtc.now()
    print('ts', ts,'tflash',  t_flash_int, '')
    lora = LoRa(mode=LoRa.LORA, frequency=863000000)
    s = socket.socket(socket.AF_LORA, socket.SOCK_RAW)
    s.setblocking(False)
    premier_capteur=1
    while True:   
        ...
        time.sleep(2)         
        if trame_ch: #      trame=label+delimiteur+str(numero_trame)+delimiteur+str(t)+delimiteur+str(w)+trame+delimiteur+"\n"       
            pycom.rgbled(0x660000)            
            time.sleep(2)         
            trame = trame_ch.decode('utf-8')#sinon pbs compatibilité avec binaire?
            trame=trame.split(delimiteur) #on vire le delimiteur et on met les data dans une liste        
            ...
            if trame[0] ==label :     #vérification champ 0  pour controle destinataire, et controle champ 1  n° trame pour prendre une seule mesure
                ...
                for i in range(premier_capteur, nombre_capteurs+premier_capteur)    :                    
                    lecture_capteur[i]=float((trame)[i+3])
                ...
                if  numero_trame!=trame_flash and transmission:
                    ...
                    flashWriteData(trame_ch)     #on sauve la trame sur flash
                    flashWriteTrame ( (trame[1]))#on écrit le nouveau numero de trame
                    flashWriteTime(ts)#on sauve le temps sur flash
            else:
                print ("erreur transmission")                
...
if  configuration == 300 : # RX: ON VA TRANSFERER le fichier data de la carte flash vers le répertoire courant si  le lopy est sur l'USB0
    os.system('ampy --port /dev/ttyUSB0 get fichier_data fichier_data_copie')
