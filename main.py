...
#on rajoute un chien de garde pour éviter les plantages réguliers (tous les 2/3 jours pour TX, un peu mieux pour RX)
import pycom
import errno
import os
...
w=c.w                                                                          #champ vide pour usage ultérieur
...
#trame enregistrée=label+delimiteur+str(numero_trame)+delimiteur+str(t)+delimiteur+w+{delimiteur+str(lecture_capteur[i])}*nb_capteurs+delimiteur+time_stamp+delimiteur+"\n"        
    ...
    while True:   
        ...
        if c.nombre_capteurs_rx!=  0 : #  ON VA lire LE RX il y a i capteurs sur le RX; il y a zero capteur sur le TX;trame=timestamp+delimiteur+str(numero_trame)+delimiteur+str(t)+6*trames+delimiteur+"\n"       
            numero_trame=0
            poids_en_gr_total=b=0
            trame=''
            poids_en_gr=poids_en_gr_total=0
            nombre_capteurs_rx=c.nombre_capteurs_rx
            premier_capteur_rx=c.premier_capteur_rx
