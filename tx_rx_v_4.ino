...
version tx_rx_v3 : on fait un cycle de mesures sur n points, b fois de suite en envoyant un numéro de trame sauvé sur EEPROM TX (on s'est aperçu qu'on perdait bcp de trames), tx_rx_v_4 : 4 capteurs mais 
mais renvoie une seule valeur globale pour économiser l'EEPROM, et 6 capteurs pour monter à 120 kg. pour 19 octets, faut émettre 3 (?) secondes, pour 7 on émet 1 seconde. Tempo démarrage, on redescend à 0.1 s.
Du coup, l'autonomie qui était tombée à 30 jours devrait remonter à 60 jours, et si on envoie une trame au lieu de trois on devrait arriver à six mois.
      ...
      case 1014: // RX ET TX, ON VA CONFIGURER LE RX; il y a 0 capteurs sur le RX; il y a 4 capteurs sur le TX; la trame reçue fait 7 octets  : 2 [label]+1 [T]+4 [1 long]
      ...
      case 106 : // RX ET TX, ON VA CONFIGURER LE RX; il y a 0 capteurs sur le RX; il y a 4 capteurs sur le TX; la trame reçue fait 19 octets : 2 [label]+1 [T]+16 [4 long]
      ...
      case 2014: // RX ET TX ON VA CONFIGURER LE TX; il y a x capteurs sur le RX; il y a 4 capteurs sur le TX; la trame émise fait 7 octets  : 2 [label]+1 [T]+4 [1* long]
      ...
      case 206 : // RX ET TX ON VA CONFIGURER LE TX; il y a x capteurs sur le RX; il y a 6 capteurs sur le TX; la trame émise fait 27 octets : 2 [label]+1 [T]+24 [6 long]
      ...
      case 306 : // RX: ON VA LIRE L'EEPROM; il y a 0 capteur sur le RX; il y a 6 capteurs sur le TX; la trame EEPROM fait 25 octets : 1 [T]+24 [6* long]
...
*/
int configuration = 110; // Choisir une configuration: 1er chiffre 1->configuration RX, 2->configurationTX, 3->lecture EEPROM/TX       2eme chiffre->nb capteurs RX       3eme chiffre->nb sur TX
...
long tempo_local=1000; //tempo entre deux mesures si capteur local seulement, sinon c'est le distant qui pilote
...
const int tempo_lora_demarrage = 100;//le temps que la carte lora soit opérationnelle
...
const byte octet_par_trame_max=27; //c'est la longueur de trame la plus longue de tous les "cases" programmés, elle sert à programmer les deux tableaux data et data_rcv
...
HX711 capteur_30kg(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_1(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 Proto alu brossé Wi6Lab
HX711 capteur_20kg_2(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_3(A1 , A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_4(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_5(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 N'EXISTE PAS
HX711 capteur_20kg_6(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 IDEM
HX711 capteur_20kg_1A(A4, A3);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 Proto alu blanc Lopy
HX711 capteur_20kg_2A(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_3A(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_4A(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_5A(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
HX711 capteur_20kg_6A(A1, A0);//HX711.DOUT- pin #A1  et  HX711.PD_SCK-  pin #A0 
...
const float mesure_20kg_4 = 613500 -tare_20kg_4; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_5 = -95950; // tare 20kg_1 : valeur ADC sans rien le 04/04/2017
const float etalon_20kg_5 = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_5 = 408200 -tare_20kg_5; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_6 = 95950; // tare 20kg_1 : valeur ADC sans rien
const float etalon_20kg_6 = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_6 = 613500 -tare_20kg_6; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare 
const float tare_20kg_1A = -44450; // tare 20kg_1 : valeur ADC sans rien
const float etalon_20kg_1A = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_1A = 478650 -tare_20kg_1A; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_2A = 95900; // tare 20kg_1 : valeur ADC sans rien le 03/09/2017
const float etalon_20kg_2A = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_2A = 613000 -tare_20kg_2A; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_3A = -95950; // tare 20kg_1 : valeur ADC sans rien le 04/04/2017
const float etalon_20kg_3A = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_3A = 408200 -tare_20kg_3A; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_4A = 95950; // tare 20kg_1 : valeur ADC sans rien le 04/04/2017
const float etalon_20kg_4A = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_4A = 613500 -tare_20kg_4A; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_5A = -95950; // tare 20kg_1 : valeur ADC sans rien le 04/04/2017
const float etalon_20kg_5A = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_5A = 408200 -tare_20kg_5A; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
const float tare_20kg_6A = 95950; // tare 20kg_1 : valeur ADC sans rien le 04/04/2017
const float etalon_20kg_6A = 5202; // etalonnage 20kg_1: poids de l'étalon en grammes
const float mesure_20kg_6A = 613500 -tare_20kg_6A; // etalonnage 20kg_1: valeur ADC avec l'étalon sur la balance moins la tare
...
long lecture_capteur_5 = 0;
long lecture_capteur_6 = 0;
...
//  EEPROM.write (1023, data[1]);//on écrit le premier numero de trame sur le RX, ou le TX???
  ...
  switch (configuration) {
    ...
    case 101 : // RX ET TX, ON VA CONFIGURER LE RX; il y a 0 capteurs sur le RX; il y a un capteur sur le TX; la trame reçue fait 7 octets : 2 [label/num trame]+1 [T]+4 [1 long]
    ...
    case 1014 : // RX ET TX, ON VA CONFIGURER LE RX; il y a 0 capteurs sur le RX; il y a quatre capteur sur le TX, on calcule poids total en g et on envoie; la trame reçue fait 7 octets : 2 [label/num trame]+1 [T]+4 [1 long]
     ...
     {//vérification octet premier  pour controle destinataire, et controle n° trame pour prendre une seule mesure
       ...
       transmission=1 ;//transmission déclarée OK
       lecture_capteur_1        =lectureLong( premier_octet, data_rcv);
       long  poids_en_gr_distant=lecture_capteur_1;Serial.print(" n°1 ");Serial.print(poids_en_gr_distant);
       numero_trame             =data_rcv[premier_octet-2];//octet deuxieme
       temperature_distant      =(data_rcv[premier_octet-1]/2.5-30);//octet troisieme
       temperature_local        =temperatureArduino(GAIN_local,OFFSET_local)/2.5-30;
       Serial.print(" poids en gr distant: ");Serial.print(poids_en_gr_distant);
...
*/
    ...
    case 106 : // RX ET TX, ON VA CONFIGURER LE RX; il y a 0 capteurs sur le RX; il y a six capteurs sur le TX; la trame reçue fait 27 octets : 1 [label] + 1 [numero_trame] +1 [T]+24 [6 long]
      available = checkifAvailableRXData() ;//on regarde si il y a des paquets LoRa en attente
      octet_par_trame=27;
      while (!available){
        available = checkifAvailableRXData() ;
     ...
     {//vérification octet premier  pour controle destinataire, et controle n° trame pour prendre une seule mesure
       ...
       lecture_capteur_4        =lectureLong( premier_octet+3*l, data_rcv);
       poids_en_gr_distant      =((lecture_capteur_4-tare_20kg_4)/mesure_20kg_4*etalon_20kg_4);Serial.print(" n°4 ");Serial.print(poids_en_gr_distant);
       lecture_capteur_5        =lectureLong( premier_octet+4*l, data_rcv);
       poids_en_gr_distant      =((lecture_capteur_5-tare_20kg_5)/mesure_20kg_5*etalon_20kg_5);Serial.print(" n°5 ");Serial.print(poids_en_gr_distant);
       lecture_capteur_6        =lectureLong( premier_octet+5*l, data_rcv);
       poids_en_gr_distant      =((lecture_capteur_6-tare_20kg_6)/mesure_20kg_6*etalon_20kg_6);Serial.print(" n°6 ");Serial.print(poids_en_gr_distant);
       poids_en_gr_distant      =((lecture_capteur_1-tare_20kg_1)/mesure_20kg_1*etalon_20kg_1)+((lecture_capteur_2-tare_20kg_2)/mesure_20kg_2*etalon_20kg_2)+((lecture_capteur_3-tare_20kg_3)/mesure_20kg_3*etalon_20kg_3)+((lecture_capteur_4-tare_20kg_4)/mesure_20kg_4*etalon_20kg_4)+((lecture_capteur_5-tare_20kg_5)/mesure_20kg_5*etalon_20kg_5)+((lecture_capteur_6-tare_20kg_6)/mesure_20kg_6*etalon_20kg_6);
       numero_trame             =data_rcv[premier_octet-2];//octet deuxieme
       temperature_distant      =(data_rcv[premier_octet-1]/2.5-30);//octet troisieme
       temperature_local        =temperatureArduino(GAIN_local,OFFSET_local)/2.5-30;
       Serial.print(" poids en gr distant: ");Serial.print(poids_en_gr_distant);
...
//      lecture_capteur_1=acquisitionCapteur10kg(tare_10kg,mesure_10kg,etalon_10kg,nombre_point);//fait l'acquisition sur le capteur 10 kg sur une moyenne de nombre_point
      lecture_capteur_1=acquisitionCapteur20kg_1A(tare_20kg_1A,mesure_20kg_1A,etalon_20kg_1A,nombre_point);//fait l'acquisition sur le capteur n°1-sérieA-20 kg sur une moyenne de nombre_point
...
case 2014 : // RX ET TX ON VA CONFIGURER LE TX; il y a x capteurs sur le RX; il y a quatre capteurs sur le TX mais on ne renvoie que la somme en grammes; la trame émise fait 7 octets : 2 [label/trame]+1 [T]+4 [1 long]
       ...
       { //on va envoyer b mesures pour savoir pourquoi la première mesure est fausse, sur capteurs 4 & 3 en particulier
        ...
        unsigned long poids_en_gr_total =((lecture_capteur_1-tare_20kg_1)/mesure_20kg_1*etalon_20kg_1)+((lecture_capteur_2-tare_20kg_2)/mesure_20kg_2*etalon_20kg_2)+((lecture_capteur_3-tare_20kg_3)/mesure_20kg_3*etalon_20kg_3)+((lecture_capteur_4-tare_20kg_4)/mesure_20kg_4*etalon_20kg_4);
        Serial.print (" poids_en_gr_total : ");  Serial.print (poids_en_gr_total);  
        data[0] = label; 
        data[1] = EEPROM.read(1023); //numero trame
        data[2] = temperatureArduino(GAIN_distant,OFFSET_distant);//mesure de la température du TX
        data[3] =  poids_en_gr_total >> 24; // il faut decomposer poids_en_gr_total  en ses 4 octets pour les passer au shield LoRa
        data[4] = (poids_en_gr_total >> 16) & 0xFF;
        data[5] = (poids_en_gr_total >>  8) & 0xFF;
        data[6] = (poids_en_gr_total      ) & 0xFF; 
      ...
      case 206 : // RX ET TX ON VA CONFIGURER LE TX; il y a x capteurs sur le RX; il y a six capteurs sur le TX; la trame émise fait 27 octets : 1 [label] + 1 n°trame +1 [T]+24 [6 long]
        octet_par_trame=27;
       while (b < b_mesure_par_cycle) 
       { //on va envoyer b mesures pour savoir pourquoi la première mesure est fausse, sur capteurs 4 & 3 en particulier
        b+=1;
        ...
        lecture_capteur_3=acquisitionCapteur20kg_3(tare_20kg_3,mesure_20kg_3,etalon_20kg_3,nombre_point);//fait l'acquisition sur le capteur 20 kg sur une moyenne de nombre_point
        lecture_capteur_4=acquisitionCapteur20kg_4(tare_20kg_4,mesure_20kg_4,etalon_20kg_4,nombre_point);//fait l'acquisition sur le capteur 20 kg sur une moyenne de nombre_point
        lecture_capteur_5=acquisitionCapteur20kg_5(tare_20kg_5,mesure_20kg_5,etalon_20kg_5,nombre_point);//fait l'acquisition sur le capteur 20 kg sur une moyenne de nombre_point
        lecture_capteur_6=acquisitionCapteur20kg_6(tare_20kg_6,mesure_20kg_6,etalon_20kg_6,nombre_point);//fait l'acquisition sur le capteur 20 kg sur une moyenne de nombre_point
        data[0] = label; 
        //data[1] = label; 
        data[1] = EEPROM.read(1023); //numero trame
        data[2] = temperatureArduino(GAIN_distant,OFFSET_distant);//mesure de la température du TX
       // data[2] = b;
        ...
        data[9]  = (lecture_capteur_2 >>  8) & 0xFF;
        data[10] = (lecture_capteur_2      ) & 0xFF;
        data[11] =  lecture_capteur_3 >> 24; // il faut decomposer lecture_capteur_3  en ses 4 octets pour les passer au shield LoRa
        data[12] = (lecture_capteur_3 >> 16) & 0xFF;
        data[13] = (lecture_capteur_3 >>  8) & 0xFF;
        data[14] = (lecture_capteur_3     ) & 0xFF; 
        data[15] =  lecture_capteur_4 >> 24; // il faut decomposer lecture_capteur_4  en ses 4 octets pour les passer au shield LoRa
        data[16] = (lecture_capteur_4 >> 16) & 0xFF;
        data[17] = (lecture_capteur_4 >>  8) & 0xFF;
        data[18] = (lecture_capteur_4     ) & 0xFF;
        data[19] =  lecture_capteur_5 >> 24; // il faut decomposer lecture_capteur_5  en ses 4 octets pour les passer au shield LoRa
        data[20] = (lecture_capteur_5 >> 16) & 0xFF;
        data[21] = (lecture_capteur_5 >>  8) & 0xFF;
        data[22] = (lecture_capteur_5     ) & 0xFF;
        data[23] =  lecture_capteur_6 >> 24; // il faut decomposer lecture_capteur_6  en ses 4 octets pour les passer au shield LoRa
        data[24] = (lecture_capteur_6 >> 16) & 0xFF;
        data[25] = (lecture_capteur_6 >>  8) & 0xFF;
        data[26] = (lecture_capteur_6     ) & 0xFF;
        arduinoLoraTXWrite (data,octet_par_trame);
        delay(tempo_lora_emission);
        Serial.print (" T°C: ");Serial.print (data[2]/2.5-30);Serial.print(" numero_trame: ");Serial.print(EEPROM.read(1023));Serial.print (" b: ");Serial.println (b);
       }
      ...
      address = adresseCourante();
       ...
       Serial.print(" poids: ");    Serial.println(lecture_capteur);    
     }
      break;
      ...
      case 306 : // RX: ON VA LIRE L'EEPROM; il y a 0 capteur sur le RX; il y a six capteurs sur le TX; la trame EEPROM fait 25 octets : 1 [T]+24 [6* long]
      octet_par_trame=25;
      lecture=lecture+1;//mode lecture
      delay (5000);
      address = adresseCourante();
       ...
       Serial.print("l_20kg_5: ");     Serial.println(lecture_capteur); 
       j=j+l;//on décale de l(4) octets
       lecture_capteur = buf[j];
           for (uint8_t i=j;i<l+j;i++) {
             lecture_capteur |= (long)buf[i] << (24 - (8*(i-j)));//on recompose le long en concaténant les 4 octets
           }
       Serial.print("l_20kg_6: ");     Serial.println(lecture_capteur); 
...
long acquisitionCapteur50kg(float tare,float mesure,float etalon, int nombre_point) {
    ...
    Serial.print(" ADC 50kg*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
 ...
 long acquisitionCapteur30kg(float tare,float mesure,float etalon, int nombre_point) {
    ...
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 30kg*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
 ...
 long acquisitionCapteur20kg_1(float tare,float mesure,float etalon, int nombre_point) {
    ...
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_1*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
 ...
 long acquisitionCapteur20kg_2(float tare,float mesure,float etalon, int nombre_point) {
    ...
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_2*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
 ...
 long acquisitionCapteur20kg_3(float tare,float mesure,float etalon, int nombre_point) {
    ...
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_3*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
 ...
 long acquisitionCapteur20kg_4(float tare,float mesure,float etalon, int nombre_point) {
    ...
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_4*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
    capteur_20kg_4.power_down();// put the ADC in sleep mode
    return lecture_capteur;
}

 long acquisitionCapteur20kg_5(float tare,float mesure,float etalon, int nombre_point) {
    capteur_20kg_5.power_up();
    delay (delai_avant_acquisition);          
    long lecture_capteur = capteur_20kg_5.read_average(nombre_point);//c'est un long moyenné sur x lectures, que l'on va envoyer sur le port serie ou sur eeprom
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_5*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
    capteur_20kg_5.power_down();// put the ADC in sleep mode
    return lecture_capteur;
}

 long acquisitionCapteur20kg_6(float tare,float mesure,float etalon, int nombre_point) {
    capteur_20kg_6.power_up();
    delay (delai_avant_acquisition);          
    long lecture_capteur = capteur_20kg_6.read_average(nombre_point);//c'est un long moyenné sur x lectures, que l'on va envoyer sur le port serie ou sur eeprom
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_6*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
    capteur_20kg_6.power_down();// put the ADC in sleep mode
    return lecture_capteur;
}

 long acquisitionCapteur20kg_1A(float tare,float mesure,float etalon, int nombre_point) {
    capteur_20kg_1A.power_up();
    delay (delai_avant_acquisition);          
    long lecture_capteur = - capteur_20kg_1A.read_average(nombre_point);//c'est un long moyenné sur x lectures, que l'on va envoyer sur le port serie ou sur eeprom
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_1A*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
    capteur_20kg_1A.power_down();// put the ADC in sleep mode
    return lecture_capteur;
} 

 long acquisitionCapteur20kg_3A(float tare,float mesure,float etalon, int nombre_point) {
    capteur_20kg_3A.power_up();
    delay (delai_avant_acquisition);          
    long lecture_capteur = -capteur_20kg_3A.read_average(nombre_point);//c'est un long moyenné sur x lectures, que l'on va envoyer sur le port serie ou sur eeprom
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_3A*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
    capteur_20kg_3A.power_down();// put the ADC in sleep mode
    return lecture_capteur;
} 
 long acquisitionCapteur20kg_6A(float tare,float mesure,float etalon, int nombre_point) {
    capteur_20kg_6A.power_up();
    delay (delai_avant_acquisition);          
    long lecture_capteur = -capteur_20kg_6A.read_average(nombre_point);//c'est un long moyenné sur x lectures, que l'on va envoyer sur le port serie ou sur eeprom
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 20kg_6A*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
    capteur_20kg_6A.power_down();// put the ADC in sleep mode
    return lecture_capteur;
} 
 ...
 long acquisitionCapteur10kg(float tare,float mesure,float etalon, int nombre_point) {
    ...
    float poids_en_gr=(lecture_capteur-tare)/mesure*etalon;
    Serial.print(" ADC 10kg*");Serial.print(lecture_capteur);Serial.print("*");Serial.print(poids_en_gr);Serial.print("*");
...
int lectureEeprom(int adresse, uint8_t buf[27],int octet_par_cycle) {//on lit une trame EEPROM qu'on met dans le tableau buf
  uint8_t i;
...
int sauverEntier(int adresse, long entier) {// //fonction d'écriture d'un long de 4 octets en mémoire EEPROM,retourne la prochaine adresse libre sauf si dépassement
  ...
  int sauverDistant(int adresse, uint8_t buf[27],int octet_par_trame ) {//fonction d'écriture de octet_par_trame octets (1 temperature, 4 capteur_1, 4 capteur_2, ...) octets en mémoire EEPROM,retourne la prochaine adresse libre sauf si dépassement
  uint8_t i=2;
