...
print (' configuration 306 : RX: ON VA LIRE la flash; il y a 0 capteur sur le RX; il y a 6 capteurs sur le TX')
...
if configuration== 101: # ON VA CONFIGURER LE RX il y a 0 capteurs sur le RX; il y a un capteur 20 kg sur le TX;   
...
if configuration ==  110 : # RX seul ON VA CONFIGURER LE RX il y a un capteur sur le RX; il y a zero capteur sur le TX;trame=timestamp+delimiteur+str(numero_trame)+delimiteur+str(t)+trame+delimiteur+"\n"       
    ...
    while True:
        ...
        while b < b_mesure_par_cycle : #on peut faire b mesures pour fiabiliser la mesure
             ...
             i =0#on fait la mesure sur le capteur_1
             lecture_capteur[i]=acquisitionCapteur_1( tare_20kg[i ], mesure_20kg [i],  etalon_20kg , nombre_point )  #fait l'acquisition sur le capteur 20 kg_i sur une moyenne de nombre_point
             poids_en_gr=((lecture_capteur[i]-tare_20kg[i])/mesure_20kg[i]*etalon_20kg)
             print(" n°", i+1, poids_en_gr)
