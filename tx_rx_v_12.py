...
print (' configuration 120 : RX seul, ON VA CONFIGURER LE RX; il y a 2 capteurs sur le RX; il y a 0 capteurs sur le TX') 
...
def acquisitionCapteur_1( tare,mesure,etalon, nombre_point) :
     ...
     poids_en_gr=(lecture_capteur-tare)/mesure*etalon
     print("   valeur ADC capteur_1 : ", lecture_capteur,"*",  poids_en_gr,"*")
     capteur_1.power_down()# put the ADC in sleep mode
     return lecture_capteur


def acquisitionCapteur_2( tare,mesure,etalon, nombre_point) :
     capteur_2.power_up()#reveille le HX711 n°2
     time.sleep (delai_avant_acquisition)
     lecture_capteur = capteur_2.read()
     poids_en_gr=(lecture_capteur-tare)/mesure*etalon
     print("   valeur ADC capteur_2 : ", lecture_capteur,"*",  poids_en_gr,"*")
     capteur_2.power_down()# put the ADC in sleep mode
     return lecture_capteur

def acquisitionCapteur_3( tare,mesure,etalon, nombre_point) :
     capteur_3.power_up()#reveille le HX711 n°3
     time.sleep (delai_avant_acquisition)
     lecture_capteur = capteur_3.read()
     poids_en_gr=(lecture_capteur-tare)/mesure*etalon
     print("   valeur ADC capteur_3 : ", lecture_capteur,"*",  poids_en_gr,"*")
     capteur_3.power_down()# put the ADC in sleep mode
     return lecture_capteur
     
def acquisitionCapteur_4( tare,mesure,etalon, nombre_point) :
     capteur_4.power_up()#reveille le HX711 n°4
     time.sleep (delai_avant_acquisition)
     lecture_capteur = capteur_4.read()
     poids_en_gr=(lecture_capteur-tare)/mesure*etalon
     print("   valeur ADC capteur_4 : ", lecture_capteur,"*",  poids_en_gr,"*")
     capteur_4.power_down()# put the ADC in sleep mode
     return lecture_capteur

def acquisitionCapteur_5( tare,mesure,etalon, nombre_point) :
     capteur_5.power_up()#reveille le HX711 n°5
     time.sleep (delai_avant_acquisition)
     lecture_capteur = capteur_5.read()
     poids_en_gr=(lecture_capteur-tare)/mesure*etalon
     print("   valeur ADC capteur_5 : ", lecture_capteur,"*",  poids_en_gr,"*")
     capteur_5.power_down()# put the ADC in sleep mode
     return lecture_capteur
     
def acquisitionCapteur_6( tare,mesure,etalon, nombre_point) :
     capteur_6.power_up()#reveille le HX711 n°6
     time.sleep (delai_avant_acquisition)
     lecture_capteur = capteur_6.read()
     poids_en_gr=(lecture_capteur-tare)/mesure*etalon
...
if configuration ==  110 : # RX seul ON VA CONFIGURER LE RX il y a un capteur sur le RX; il y a zero capteur sur le TX;trame=timestamp+delimiteur+str(numero_trame)+delimiteur+str(t)+trame+delimiteur+"\n"       
    ...
    while True:
        ...
        while b < b_mesure_par_cycle : #on peut faire b mesures pour fiabiliser la mesure
             ...
             i =0#on fait la mesure sur le capteur_i +1
             lecture_capteur[i]=0
             moyenne=0
             j=0                 
             n=0
             while j < nombre_point and n< nombre_point:
                 ...
                 j+=1
             if n>=nombre_point:
                 print ('beaucoup d\'erreurs capteur n°', i+1)
             lecture_capteur[i]=moyenne/j#moyenne attention à division par zero*************************************
...
if configuration ==  120 : # RX seul ON VA CONFIGURER LE RX il y a deux capteurs sur le RX; il y a zero capteur sur le TX;trame=timestamp+delimiteur+str(numero_trame)+delimiteur+str(t)+trame+delimiteur+"\n"       
    numero_trame=0
    ...
    while True:
        ...
        while b < b_mesure_par_cycle : #on peut faire b mesures pour fiabiliser la mesure
            b+=1
            poids_en_gr=poids_en_gr_total=0
            i =2#on fait la mesure sur les capteur_i (3 et 4 ici)
            lecture_capteur[i]=0
            moyenne=0
            j=0                 
            n=0
            while j < nombre_point and n< nombre_point:
                 lecture_capteur[i]=acquisitionCapteur_3( tare_20kg[i ], mesure_20kg [i],  etalon_20kg , nombre_point )  #fait l'acquisition sur le capteur 20 kg_i fois le  nombre_point
                 moyenne+=lecture_capteur[i]
                 if lecture_capteur[i] in liste:
                     moyenne-=lecture_capteur[i]                     
                     print("liste")
            ...
            poids_en_gr_total=poids_en_gr+poids_en_gr_total
            print(" n°", i+1, poids_en_gr,"  ", lecture_capteur[i] )
            lecture_capteur[i+1]=0            
            moyenne=0
            j=0                 
            n=0
            while j < nombre_point and n< nombre_point:
                 lecture_capteur[i+1]=acquisitionCapteur_4( tare_20kg[i+1 ], mesure_20kg [i+1],  etalon_20kg , nombre_point )  #fait l'acquisition sur le capteur 20 kg_i fois le  nombre_point
                 moyenne+=lecture_capteur[i+1]
                 if lecture_capteur[i+1] in liste:
                     moyenne-=lecture_capteur[i+1]                     
                     print("liste")
                     j-=1
                     n+=1
                 j+=1
            if n>=nombre_point:
                 print ('beaucoup d\'erreurs capteur n°', i+1+1)
            lecture_capteur[i+1]=moyenne/j#moyenne attention à division par zero*************************************
            poids_en_gr=((lecture_capteur[i+1]-tare_20kg[i+1])/mesure_20kg[i+1]*etalon_20kg)        
            print(" n°", i+1+1, poids_en_gr,"  ", lecture_capteur[i+1] )            
            poids_en_gr_total=poids_en_gr+poids_en_gr_total
        numero_trame+=1 
        t=temperatureLopy(GAIN_local,OFFSET_local)#mesure de la température du TX***************************************************************************    
        timest = time.localtime()#on met un timestamp sur la trame
        timest=str(timest)   
        trame=timest+delimiteur+str(numero_trame)+delimiteur+str(t)+delimiteur+str(lecture_capteur[i])+delimiteur+str(lecture_capteur[i+1])+delimiteur+"\n"
        flashWriteData(trame)
        print("poids_en_gr_total", poids_en_gr_total, " Température RX: ", t,  " n° trame: ", numero_trame, "trame   ", trame)
        b=0        
    time.sleep(delai_local)
