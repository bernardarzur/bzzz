...
#v_18 : on fait trois mains, un RX (70 lignes), un TX(110 lignes) et un RX_TX (250 lignes), pour réduire le temps de compilation. On passe de 3.8s à 3.5s puis 2.6s. 10% gain sur TX -> bof, bof, le RX ne sert
#à rien et le TX? on rajoute un capteur temperature/humidity DHT11 : compilation 4.5s + mesure 2.5s=7s
import pycom
import os
...
capteur_temp=c.capteur_temp   #'DHT11'                                        #capteur temperature sur TX
...
while True:   
    ...
    #trame=[label+delimiteur]+str(t)+delimiteur+w+{delimiteur+str(lecture_capteur[i])}*nombre_capteurs+delimiteur+"\n"; la trame RAW nécessite un "label" pour identification sans ambigüité
        ...
        if capteur_temp=='DHT11' :            
            th = dth.DTH(Pin(c.pin_temp, mode=Pin.OPEN_DRAIN),0)
            time.sleep(c.delai_flash_mise_en_route)
            result = th.read()
            if result.is_valid():
                ...
                temp=hum=0
            trame=trame+str(temp)+delimiteur +str(hum)+delimiteur
        if debug:
